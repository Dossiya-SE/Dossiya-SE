...
3. **The computed gold resilience-margin vector is the Euclidean L2 margin `ρ₂`; it must be mathematically normal to the active critical boundary. A general `ρ_g` requires a separately justified metric.**
...
10. **Computed numerical artifacts are canonically serialized.** Double-precision computations are emitted at the governed 12-significant-digit representation with values below (10^{-14}) canonicalized to zero. CI also runs BLAS/LAPACK single-threaded. Numerically equivalent runs must therefore produce byte-identical computed JSON, TikZ and SVG artifacts.
11. **G6 is a true double-build byte test.** The release hashes the computed JSON, Julia audit, layout metadata, TikZ source, all 10 final SVGs and all 28 runtime PNG previews, rebuilds the V3 chain, and requires all 42 SHA-256 hashes to remain identical.
