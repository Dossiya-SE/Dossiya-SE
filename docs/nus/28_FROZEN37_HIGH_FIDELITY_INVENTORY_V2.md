...
The inventory preserves the frozen canonical 37-paper order and extends the paper-level `Q → I → M_E → P → F → M_S → S → D` architecture with explicit Source Map, EvidenceAnchor, Observation, Transformation, Surface/Geometry, Validation, Decision, QA, and Assurance layers.

## Exact artifact identities

The exact session artifacts are hash-locked as follows:

- `NUS_FROZEN37_HIGH_FIDELITY_INVENTORY_V2.xlsx`
  - bytes: `214100`
  - SHA-256: `55fc37bebb7c370a670bce3ae56b9f3d78d689d8ac67ac93a3fb238105569ad3`
- `NUS_FROZEN37_HIGH_FIDELITY_INVENTORY_V2.json`
  - bytes: `218568`
  - SHA-256: `15320e1b80efb9810fe7da54d94bfeb2124735b5db961da66e85a0954dc37b70`
- Combined ZIP checkpoint containing those two exact files:
  - bytes: `176273`
  - SHA-256: `39876e0a84feeebb943516150b990b9ace9867c7bf3422b3d759d15197c8c77e`

## GitHub machine checkpoint

The repository contains the compact machine-verifiable checkpoint:

`docs/nus/machine/frozen37_high_fidelity_inventory_v2_manifest.json`

It freezes:

- paper count = 37;
- canonical Frozen-37 order;
- governing extraction chain;
- current engine = `NUS_EVIDENCE_ENGINE_V1.0.2`;
- current permanent regression boundary = `T001–T049`;
- candidate controls = `T050`, `T051`;
- exact XLSX/JSON/ZIP byte counts and SHA-256 values;
- current paper-authority boundary;
- critical fail-closed scientific rules;
- mutation authorization = false.

The connected GitHub contents interface used in this session is text-oriented. Therefore the binary XLSX and large full JSON are **not represented as silently transformed repository files**. Their exact identities are preserved by the hashes above and by the committed machine manifest.
...
- NUS-172: `CLOSED_GOLD_REFERENCE`.
- NUS-48: `FROZEN_CURRENT`.
...
This inventory checkpoint is documentation/data authority only. It does not authorize Zotero annotation creation, deletion, comment mutation, geometry mutation, PDF mutation, or a final-schema write.
